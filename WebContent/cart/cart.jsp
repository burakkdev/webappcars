      ...
      <c:otherwise>
        <table>
          ...
          <c:forEach var="item" items="${cart.items}">
            <tr class="cart_row">
              <td>
                <form action="<c:url value='/orderPanel/updateItem'/>" method="post">
              ...
              <td>
                <form action="<c:url value='/orderPanel/removeItem'/>" method="post">
                  <input type="hidden" name="productCode" 
                         value="<c:out value='${item.product.code}'/>">
...
<form action="<c:url value='/carbrowse'/>" method="get" id="float_left">
  <input type="submit" value="Continue Shopping">
</form>
  
<c:if test="${emptyCart == null}">
    <!-- Connection is NOT SECURE.  For testing only. -->
    <form action="<c:url value='/orderPanel/checkUser'/>" method="post">
